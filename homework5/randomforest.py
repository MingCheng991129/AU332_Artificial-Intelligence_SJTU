...
def randomForest1(features, labels, features_test, numberOfTree):
    ...
    if os.path.exists("predictef.csv") == False:

        with open("predictef.csv", "w", newline='') as csvfile:
...
if __name__ == '__main__':
    ...
    score = []
    num = []
    randomForest1(features, labels, features_test, 50)
    ...
    for numTree in range(1000, 1500, 50):
        num.append(numTree)
        ...
        for train_index, test_index in ss.split(features):
            ...
            score_per_lis.append(randomForest(train_X, train_y, test_X, test_y, numTree))
